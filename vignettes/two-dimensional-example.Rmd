...
CatSIM compares two different labelings of pixels (voxels) in 2-D (or 3-D) images
in an image. It attempts to assess similarity between two images that accounts for the similarity between
...
CatSIM computes a set of similarity metrics on a sliding window on the image, then
averages and combines them. Then it downsamples to another layer and repeats the process.
...
main comparison between the two images. However, there are several other options
