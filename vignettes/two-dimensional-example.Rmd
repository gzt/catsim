...
It works by assessing similarity within a sliding window across the image,
then downsampling and doing the same on different scales, and then combining
the ratings for each scale together. The first scale is the entire image, the second scale is 
the image shrunk down to half its size, and so on.
...
are approximatly the same for the two, an image similarity metric should
...
"ground truth" image.
```{r construction, fig.width=6, fig.height=6}
...
catsim(basemat, shiftmat, weights = rep(.2,5), method="NMI")
