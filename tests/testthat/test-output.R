...
test_that("dimensions 2D work", {
  ...
  expect_error(AdjustedRand(x, y[, 1:10]))
  expect_error(RandIndex(x, y[, 1:10]))
  expect_error(normalizedMI(x, y[, 1:10]))
  expect_error(adjustedMI(x, y[, 1:10]))
  expect_error(CohenKappa(x, y[, 1:10]))
  expect_error(catmssim_2d(x, y[1:10, ]))
...
test_that("Inputs are symmetric 2D", {
  ...
  expect_equal(
    catmssim_2d(x, y, weights = c(.5, .5), method = "j"),
...
test_that("catmssim_2d equivalent to catsim in 2D", {
  ...
  expect_equal(
    catmssim_2d(x, y, weights = c(.23, 77)),
...
test_that("Inputs are symmetric 3D", {
  ...
  expect_equal(
    catmssim_3d_slice(x, y, weights = c(.75, .25), window = 2),
...
test_that("dimensions 3D work", {
  ...
  expect_equal(catsim(x, x,
    weights = c(.5, .5),
    window = c(5, 5, 5), method = "accuracy"
  ), 1.0)
  expect_equal(catsim(x, x,
    weights = c(.5, .5),
    window = c(5, 5, 5), method = "NMI"
  ), 1.0)
